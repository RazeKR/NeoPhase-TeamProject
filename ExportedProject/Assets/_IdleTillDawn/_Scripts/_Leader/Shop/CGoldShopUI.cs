...
public class CGoldShopUI : MonoBehaviour
{
    ...
    // 펫 상자 다이아 상품 (0~2번 버튼): 수량, 다이아 비용
    private static readonly int[] PetBoxDiamondAmounts = { 10, 25, 50 };
    private static readonly int[] PetBoxDiamondCosts   = { 10000, 25000, 50000 };
    ...
    [Header("PetShopPanel (펫 패키지 탭)")]
    [SerializeField] private GameObject _petShopPanel = null;

    [Header("펫 상자 구매 버튼 3개 (0: 10개 / 1: 25개 / 2: 50개)")]
    [SerializeField] private Button[] _petShopButtons = new Button[3];

    [Header("펫 상자 버튼별 비용 Text")]
    [SerializeField] private Text[] _petShopCostTexts = new Text[3];
    ...
    private void Start()
    {
        ...
        RegisterPetShopButtons();
        RegisterWeaponShopButtons();

        // CGoldManager 재화 변경 이벤트 구독 — 재화가 바뀌면 펫/무기/아이템 상점 버튼 상태를 즉시 갱신
        _onGoldChanged    = _ => { RefreshWeaponShopButtonStates(); RefreshItemShopButtonStates(); };
        _onDiamondChanged = _ => { RefreshPetShopButtonStates(); RefreshWeaponShopButtonStates(); };
        ...
        // 펫/무기/아이템 상점 버튼은 CGoldManager.Start()가 끝난 뒤 첫 재화 이벤트로 갱신되므로
        // 여기서는 패널이 비활성이어도 상관없지만, 이미 로드된 경우를 위해 한 번 호출
        RefreshPetShopButtonStates();
    ...
    private void OnDestroy()
    {
        ...
        for (int i = 0; i < _petShopButtons.Length; i++)
        {
            if (_petShopButtons[i] != null)
                _petShopButtons[i].onClick.RemoveAllListeners();
        }
    ...
    /// <summary>펫 상자 보유 수량 변경 시 발행됩니다. 인자: 변경 후 보유 수량</summary>
    public static event Action<int> OnPetBoxCountChanged;
    ...
    private void SelectCategory(int index)
    {
        ...
        if (_petShopPanel != null)
        {
            _petShopPanel.SetActive(index == 1);
            if (index == 1)
                RefreshPetShopButtonStates();
        }
    ...
    private void RegisterPetShopButtons()
    {
        for (int i = 0; i < _petShopButtons.Length; i++)
        {
            if (_petShopButtons[i] == null) continue;
            int captured = i;
            _petShopButtons[i].onClick.AddListener(() => TryBuyPetBoxWithDiamond(captured));
        }
    }

    /// <summary>다이아로 펫 상자 구매 (0: 10개 / 1: 25개 / 2: 50개)</summary>
    private void TryBuyPetBoxWithDiamond(int itemIndex)
    {
        if (CGoldManager.Instance == null)
        {
        ...
        int cost = PetBoxDiamondCosts[itemIndex];
        ...
        int amount = PetBoxDiamondAmounts[itemIndex];
        data.petBoxCount += amount;
        SaveData(data);

        OnPetBoxCountChanged?.Invoke(data.petBoxCount);
        RefreshPetShopButtonStates();
        CDebug.Log($"[CGoldShopUI] 다이아 {cost:N0} 소모 → 펫 상자 {amount}개 지급 (총 {data.petBoxCount}개)");
    }
    ...
    /// <summary>펫 상점 버튼 3개의 interactable 상태와 비용 텍스트 색깔을 갱신합니다.</summary>
    private void RefreshPetShopButtonStates()
    {
        if (CGoldManager.Instance == null) return;

        for (int i = 0; i < PetBoxDiamondCosts.Length; i++)
        {
            bool canBuy = CGoldManager.Instance.Diamond >= PetBoxDiamondCosts[i];
            SetPetShopButtonState(i, canBuy);
            SetPetShopCostTextColor(i, !canBuy);
        }
    }

    private void SetPetShopButtonState(int index, bool interactable)
    {
        if (index >= _petShopButtons.Length || _petShopButtons[index] == null) return;
        _petShopButtons[index].interactable = interactable;
        ColorBlock cb = _petShopButtons[index].colors;
        cb.disabledColor = Color.white;
        _petShopButtons[index].colors = cb;
    }

    // 텍스트 내용은 변경하지 않고 색깔만 변경합니다.
    private void SetPetShopCostTextColor(int index, bool insufficient)
    {
        if (index >= _petShopCostTexts.Length || _petShopCostTexts[index] == null) return;
        _petShopCostTexts[index].color = insufficient ? Color.red : Color.yellow;
    }
